...
async function main() {
  ...
  // The question lists directory
  const questionsDir = fs.existsSync(path.resolve(__dirname, '../../question-lists'))
    ? path.resolve(__dirname, '../../question-lists')
    : path.resolve(__dirname, '../../');
  console.log(`Question lists path: ${questionsDir}`);

  // List all company directories
  const allItems = fs.readdirSync(questionsDir);
  ...
  for (const slug of companySlugs) {
    const companyPath = path.join(questionsDir, slug);
